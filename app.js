...
/* ---------------------------------------------------------------------
   Tiny IndexedDB key-value + table helper
   --------------------------------------------------------------------- */
...
const state = {
  ...
  settings: { light: false, ember: true, resume: true },
  usingFSApi: false,
...
/* ---------------------------------------------------------------------
   Ember background canvas (ambient effect, matches Iron-Throne palette)
   --------------------------------------------------------------------- */
(function emberFX() {
  ...
  function loop(t) {
    ...
    if (!state.settings.ember) { ctx.clearRect(0,0,w,h); return; }
    if (t - last < 33) return; // ~30fps
    last = t;
    ctx.clearRect(0, 0, w, h);
    if (Math.random() < 0.5 && particles.length < 70) spawn();
    ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() || "#C9A84C";
...
function applySettingsToUI() {
  ...
  els.emberSwitch.classList.toggle("on", state.settings.ember);
  els.resumeSwitch.classList.toggle("on", state.settings.resume);
}
function openSettings() { els.settingsModalOverlay.classList.add("open"); }
...
els.emberSwitch.addEventListener("click", () => { state.settings.ember = !state.settings.ember; applySettingsToUI(); saveSettings(); });
